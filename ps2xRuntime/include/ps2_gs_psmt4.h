...
/**
 * PSMT4 block layout tables and address computation.
 * Based on PCSX2 GSdx / DobieStation algorithms (blockTable4, columnTable4).
 */
namespace GSPSMT4
{

// blockTable4[y][x] - 8x4, indices for block positioning
...
// columnTable4[y][x] - 16x32, column offsets within block
...
/** blockId for PSMT4 - block is dbp (256-byte blocks), width is tbw (64-pixel units) */
...
/** Nibble address in VRAM for pixel (x,y) in PSMT4 layout (DobieStation/PCSX2 convention).
 *  Use byteOff=ret>>1, nibbleShift=(ret&1)<<2 for read/write. block=dbp, width=tbw.
 *  Page size 8KB (32 blocks); page=(block>>5)+(y>>7)*(width>>1)+(x>>7). */
