// VAG (PlayStation ADPCM) decoder
// Format: 48-byte header ("VAGp"), 16-byte blocks (2 header bytes + 14 data bytes = 28 samples)
// Block format per PS1 SPU: byte0 = shift(0-3) | filter(4-6), byte1 = loop flags, bytes 2-15 = 28 nibbles
...
namespace ps2_vag
{
    // Decode VAG data to 16-bit mono PCM. Returns true on success.
                ...
                std::vector<int16_t>& outPcm, uint32_t& outSampleRate)
    {
        ...
        // Header: BE: size@0x0c, sampleRate@0x10
