...
void setPRMODECONT_0x11b8b0(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
#ifdef _DEBUG
    PS_LOG_ENTRY("setPRMODECONT_0x11b8b0");
#endif

    ctx->pc = 0x11b8b0u;

    // 0x11b8b0: 0x5283c  dsll32      $a1, $a1, 0
    ctx->pc = 0x11b8b0u;
    SET_GPR_U64(ctx, 5, GPR_U64(ctx, 5) << (32 + 0));
    // 0x11b8b4: 0x27bdfff0  addiu       $sp, $sp, -0x10
    ctx->pc = 0x11b8b4u;
    SET_GPR_S32(ctx, 29, (int32_t)ADD32(GPR_U32(ctx, 29), 4294967280));
    // 0x11b8b8: 0x5303e  dsrl32      $a2, $a1, 0
    ctx->pc = 0x11b8b8u;
    SET_GPR_U64(ctx, 6, GPR_U64(ctx, 5) >> (32 + 0));
    // 0x11b8bc: 0xffbf0000  sd          $ra, 0x0($sp)
    ctx->pc = 0x11b8bcu;
    WRITE64(ADD32(GPR_U32(ctx, 29), 0), GPR_U64(ctx, 31));
    // 0x11b8c0: 0xc046d3a  jal         func_11B4E8
    ctx->pc = 0x11B8C0u;
    SET_GPR_U32(ctx, 31, 0x11B8C8u);
    ctx->pc = 0x11B8C4u;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x11B8C0u;
            // 0x11b8c4: 0x2405001a  addiu       $a1, $zero, 0x1A (Delay Slot)
    ...
    ctx->pc = 0x11B8C8u;
    // 0x11b8c8: 0xdfbf0000  ld          $ra, 0x0($sp)
    ctx->pc = 0x11b8c8u;
    SET_GPR_U64(ctx, 31, READ64(ADD32(GPR_U32(ctx, 29), 0)));
    // 0x11b8cc: 0x3e00008  jr          $ra
    ctx->pc = 0x11B8CCu;
    {
        uint32_t jumpTarget = GPR_U32(ctx, 31);
        ctx->pc = 0x11B8D0u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x11B8CCu;
            // 0x11b8d0: 0x27bd0010  addiu       $sp, $sp, 0x10 (Delay Slot)
