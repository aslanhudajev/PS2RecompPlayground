#include "ps2_runtime_macros.h"
#include "ps2_runtime.h"
#include "ps2_recompiled_functions.h"
#include "ps2_recompiled_stubs.h"

#include "ps2_syscalls.h"
#include "ps2_stubs.h"
#include "ps2_init_log.h"

// Function: _ri0_101
// Address: 0x15f9d0 - 0x15fab4
void _ri0_101_0x15f9d0(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    PS2_INIT_LOG_ENTRY("_ri0_101");


    ctx->pc = 0x15f9d0u;

    // 0x15f9d0: 0x7000cce9
    ctx->pc = 0x15f9d0u;
    SET_GPR_VEC(ctx, 25, PS2_PNOR(GPR_VEC(ctx, 0), GPR_VEC(ctx, 0)));
    // 0x15f9d4: 0x7019cbf6
    ctx->pc = 0x15f9d4u;
    SET_GPR_VEC(ctx, 25, _mm_srli_epi16(GPR_VEC(ctx, 25), 15));
    // 0x15f9d8: 0x8c850014
    ctx->pc = 0x15f9d8u;
    SET_GPR_U32(ctx, 5, READ32(ADD32(GPR_U32(ctx, 4), 20)));
    // 0x15f9dc: 0x8c860018
    ctx->pc = 0x15f9dcu;
    SET_GPR_U32(ctx, 6, READ32(ADD32(GPR_U32(ctx, 4), 24)));
    // 0x15f9e0: 0x8c8e0000
    ctx->pc = 0x15f9e0u;
    SET_GPR_U32(ctx, 14, READ32(ADD32(GPR_U32(ctx, 4), 0)));
    // 0x15f9e4: 0x8c8d0004
    ctx->pc = 0x15f9e4u;
    SET_GPR_U32(ctx, 13, READ32(ADD32(GPR_U32(ctx, 4), 4)));
    // 0x15f9e8: 0x8c8c0010
    ctx->pc = 0x15f9e8u;
    SET_GPR_U32(ctx, 12, READ32(ADD32(GPR_U32(ctx, 4), 16)));
    // 0x15f9ec: 0x240b0001
    ctx->pc = 0x15f9ecu;
    SET_GPR_S32(ctx, 11, ADD32(GPR_U32(ctx, 0), 1));
    // 0x15f9f0: 0xcc040
    ctx->pc = 0x15f9f0u;
    SET_GPR_U32(ctx, 24, SLL32(GPR_U32(ctx, 12), 1));
    // 0x15f9f4: 0x5b80000
    ctx->pc = 0x15f9f4u;
    ctx->sa = (GPR_U32(ctx, 13) + 0) & 0xF;
label_15f9f8:
    // 0x15f9f8: 0x8c870008
    ctx->pc = 0x15f9f8u;
    SET_GPR_U32(ctx, 7, READ32(ADD32(GPR_U32(ctx, 4), 8)));
    // 0x15f9fc: 0xdca80000
    ctx->pc = 0x15f9fcu;
    SET_GPR_U64(ctx, 8, READ64(ADD32(GPR_U32(ctx, 5), 0)));
    // 0x15fa00: 0xdcc90000
    ctx->pc = 0x15fa00u;
    SET_GPR_U64(ctx, 9, READ64(ADD32(GPR_U32(ctx, 6), 0)));
    // 0x15fa04: 0x71284389
    ctx->pc = 0x15fa04u;
    SET_GPR_VEC(ctx, 8, _mm_unpacklo_epi64(GPR_VEC(ctx, 9), GPR_VEC(ctx, 8)));
    // 0x15fa08: 0x710846e8
    ctx->pc = 0x15fa08u;
    { 
    __m128i val_rt = GPR_VEC(ctx, 8);
    __m128i val_rs = GPR_VEC(ctx, 8);
    uint32_t shift_amount = ctx->sa & 0x7F; 
    uint64_t rt_hi = _mm_cvtsi128_si64(_mm_srli_si128(val_rt, 8));
    uint64_t rt_lo = _mm_cvtsi128_si64(val_rt);
    uint64_t rs_hi = _mm_cvtsi128_si64(_mm_srli_si128(val_rs, 8));
    uint64_t rs_lo = _mm_cvtsi128_si64(val_rs);
    __m128i result; 
    if (shift_amount == 0) { 
        result = val_rs; 
    } else if (shift_amount < 64) { 
        uint64_t res_lo = (rs_lo >> shift_amount) | (rs_hi << (64 - shift_amount)); 
        uint64_t res_hi = (rs_hi >> shift_amount) | (rt_lo << (64 - shift_amount)); 
        result = _mm_set_epi64x(res_hi, res_lo); 
    } else if (shift_amount == 64) { 
        result = _mm_set_epi64x(rt_lo, rs_hi); 
    } else if (shift_amount < 128) { 
        uint32_t sub_shift = shift_amount - 64; 
        uint64_t res_lo = (rs_hi >> sub_shift) | (rt_lo << (64 - sub_shift)); 
        uint64_t res_hi = (rt_lo >> sub_shift) | (rt_hi << (64 - sub_shift)); 
        result = _mm_set_epi64x(res_hi, res_lo); 
    } else { // shift_amount >= 128 
         uint32_t sub_shift = shift_amount - 128; 
         uint64_t res_lo = (rt_lo >> sub_shift) | (rt_hi << (64 - sub_shift)); 
         uint64_t res_hi = (rt_hi >> sub_shift); 
         result = _mm_set_epi64x(res_hi, res_lo); 
    } 
    SET_GPR_VEC(ctx, 8, result); 
}
    // 0x15fa0c: 0x356b8000
    ctx->pc = 0x15fa0cu;
    SET_GPR_U32(ctx, 11, OR32(GPR_U32(ctx, 11), 32768));
    // 0x15fa10: 0x10e00016
    ctx->pc = 0x15FA10u;
    {
        const bool branch_taken_0x15fa10 = (GPR_U32(ctx, 7) == GPR_U32(ctx, 0));
        ctx->pc = 0x15FA14u;
        SET_GPR_VEC(ctx, 15, PS2_PEXTLB(GPR_VEC(ctx, 0), GPR_VEC(ctx, 8)));
        if (branch_taken_0x15fa10) {
            ctx->pc = 0x15FA6Cu;
            goto label_15fa6c;
        }
    }
    ctx->pc = 0x15FA18u;
label_15fa18:
    // 0x15fa18: 0xac2821
    ctx->pc = 0x15fa18u;
    SET_GPR_U32(ctx, 5, ADD32(GPR_U32(ctx, 5), GPR_U32(ctx, 12)));
    // 0x15fa1c: 0xcc3021
    ctx->pc = 0x15fa1cu;
    SET_GPR_U32(ctx, 6, ADD32(GPR_U32(ctx, 6), GPR_U32(ctx, 12)));
    // 0x15fa20: 0xdca80000
    ctx->pc = 0x15fa20u;
    SET_GPR_U64(ctx, 8, READ64(ADD32(GPR_U32(ctx, 5), 0)));
    // 0x15fa24: 0xdcc90000
    ctx->pc = 0x15fa24u;
    SET_GPR_U64(ctx, 9, READ64(ADD32(GPR_U32(ctx, 6), 0)));
    // 0x15fa28: 0x71284389
    ctx->pc = 0x15fa28u;
    SET_GPR_VEC(ctx, 8, _mm_unpacklo_epi64(GPR_VEC(ctx, 9), GPR_VEC(ctx, 8)));
    // 0x15fa2c: 0x710846e8
    ctx->pc = 0x15fa2cu;
    { 
    __m128i val_rt = GPR_VEC(ctx, 8);
    __m128i val_rs = GPR_VEC(ctx, 8);
    uint32_t shift_amount = ctx->sa & 0x7F; 
    uint64_t rt_hi = _mm_cvtsi128_si64(_mm_srli_si128(val_rt, 8));
    uint64_t rt_lo = _mm_cvtsi128_si64(val_rt);
    uint64_t rs_hi = _mm_cvtsi128_si64(_mm_srli_si128(val_rs, 8));
    uint64_t rs_lo = _mm_cvtsi128_si64(val_rs);
    __m128i result; 
    if (shift_amount == 0) { 
        result = val_rs; 
    } else if (shift_amount < 64) { 
        uint64_t res_lo = (rs_lo >> shift_amount) | (rs_hi << (64 - shift_amount)); 
        uint64_t res_hi = (rs_hi >> shift_amount) | (rt_lo << (64 - shift_amount)); 
        result = _mm_set_epi64x(res_hi, res_lo); 
    } else if (shift_amount == 64) { 
        result = _mm_set_epi64x(rt_lo, rs_hi); 
    } else if (shift_amount < 128) { 
        uint32_t sub_shift = shift_amount - 64; 
        uint64_t res_lo = (rs_hi >> sub_shift) | (rt_lo << (64 - sub_shift)); 
        uint64_t res_hi = (rt_lo >> sub_shift) | (rt_hi << (64 - sub_shift)); 
        result = _mm_set_epi64x(res_hi, res_lo); 
    } else { // shift_amount >= 128 
         uint32_t sub_shift = shift_amount - 128; 
         uint64_t res_lo = (rt_lo >> sub_shift) | (rt_hi << (64 - sub_shift)); 
         uint64_t res_hi = (rt_hi >> sub_shift); 
         result = _mm_set_epi64x(res_hi, res_lo); 
    } 
    SET_GPR_VEC(ctx, 8, result); 
}
    // 0x15fa30: 0x70085688
    ctx->pc = 0x15fa30u;
    SET_GPR_VEC(ctx, 10, PS2_PEXTLB(GPR_VEC(ctx, 0), GPR_VEC(ctx, 8)));
    // 0x15fa34: 0x20e7ffff
    ctx->pc = 0x15fa34u;
    { uint32_t tmp; bool ov; ADD32_OV(GPR_U32(ctx, 7), (int32_t)4294967295, tmp, ov); if (ov) runtime->SignalException(ctx, EXCEPTION_INTEGER_OVERFLOW); else SET_GPR_S32(ctx, 7, (int32_t)tmp); }
    // 0x15fa38: 0x714f4908
    ctx->pc = 0x15fa38u;
    SET_GPR_VEC(ctx, 9, PS2_PADDH(GPR_VEC(ctx, 10), GPR_VEC(ctx, 15)));
    // 0x15fa3c: 0x71407ca9
    ctx->pc = 0x15fa3cu;
    SET_GPR_VEC(ctx, 15, PS2_POR(GPR_VEC(ctx, 10), GPR_VEC(ctx, 0)));
    // 0x15fa40: 0x71395108
    ctx->pc = 0x15fa40u;
    SET_GPR_VEC(ctx, 10, PS2_PADDH(GPR_VEC(ctx, 9), GPR_VEC(ctx, 25)));
    // 0x15fa44: 0x700a5076
    ctx->pc = 0x15fa44u;
    SET_GPR_VEC(ctx, 10, _mm_srli_epi16(GPR_VEC(ctx, 10), 1));
    // 0x15fa48: 0x79c80000
    ctx->pc = 0x15fa48u;
    SET_GPR_VEC(ctx, 8, READ128(ADD32(GPR_U32(ctx, 14), 0)));
    // 0x15fa4c: 0x71485108
    ctx->pc = 0x15fa4cu;
    SET_GPR_VEC(ctx, 10, PS2_PADDH(GPR_VEC(ctx, 10), GPR_VEC(ctx, 8)));
    // 0x15fa50: 0x71404988
    ctx->pc = 0x15fa50u;
    SET_GPR_VEC(ctx, 9, PS2_PCGTH(GPR_VEC(ctx, 10), GPR_VEC(ctx, 0)));
    // 0x15fa54: 0x70094bf6
    ctx->pc = 0x15fa54u;
    SET_GPR_VEC(ctx, 9, _mm_srli_epi16(GPR_VEC(ctx, 9), 15));
    // 0x15fa58: 0x71495108
    ctx->pc = 0x15fa58u;
    SET_GPR_VEC(ctx, 10, PS2_PADDH(GPR_VEC(ctx, 10), GPR_VEC(ctx, 9)));
    // 0x15fa5c: 0x700a5076
    ctx->pc = 0x15fa5cu;
    SET_GPR_VEC(ctx, 10, _mm_srli_epi16(GPR_VEC(ctx, 10), 1));
    // 0x15fa60: 0x7dca0000
    ctx->pc = 0x15fa60u;
    WRITE128(ADD32(GPR_U32(ctx, 14), 0), GPR_VEC(ctx, 10));
    // 0x15fa64: 0x1ce0ffec
    ctx->pc = 0x15FA64u;
    {
        const bool branch_taken_0x15fa64 = (GPR_S32(ctx, 7) > 0);
        ctx->pc = 0x15FA68u;
        SET_GPR_U32(ctx, 14, ADD32(GPR_U32(ctx, 14), GPR_U32(ctx, 24)));
        if (branch_taken_0x15fa64) {
            ctx->pc = 0x15FA18u;
            goto label_15fa18;
        }
    }
    ctx->pc = 0x15FA6Cu;
label_15fa6c:
    // 0x15fa6c: 0x700b53f7
    ctx->pc = 0x15fa6cu;
    SET_GPR_VEC(ctx, 10, _mm_srai_epi16(GPR_VEC(ctx, 11), 15));
    // 0x15fa70: 0x24a50140
    ctx->pc = 0x15fa70u;
    SET_GPR_S32(ctx, 5, ADD32(GPR_U32(ctx, 5), 320));
    // 0x15fa74: 0x8c87000c
    ctx->pc = 0x15fa74u;
    SET_GPR_U32(ctx, 7, READ32(ADD32(GPR_U32(ctx, 4), 12)));
    // 0x15fa78: 0x24c60140
    ctx->pc = 0x15fa78u;
    SET_GPR_S32(ctx, 6, ADD32(GPR_U32(ctx, 6), 320));
    // 0x15fa7c: 0x1475024
    ctx->pc = 0x15fa7cu;
    SET_GPR_U32(ctx, 10, AND32(GPR_U32(ctx, 10), GPR_U32(ctx, 7)));
    // 0x15fa80: 0x1540ffe5
    ctx->pc = 0x15FA80u;
    {
        const bool branch_taken_0x15fa80 = (GPR_U32(ctx, 10) != GPR_U32(ctx, 0));
        ctx->pc = 0x15FA84u;
        SET_GPR_U32(ctx, 11, AND32(GPR_U32(ctx, 11), 32767));
        if (branch_taken_0x15fa80) {
            ctx->pc = 0x15FA18u;
            goto label_15fa18;
        }
    }
    ctx->pc = 0x15FA88u;
    // 0x15fa88: 0x8c850014
    ctx->pc = 0x15fa88u;
    SET_GPR_U32(ctx, 5, READ32(ADD32(GPR_U32(ctx, 4), 20)));
    // 0x15fa8c: 0x8c860018
    ctx->pc = 0x15fa8cu;
    SET_GPR_U32(ctx, 6, READ32(ADD32(GPR_U32(ctx, 4), 24)));
    // 0x15fa90: 0x8c8e0000
    ctx->pc = 0x15fa90u;
    SET_GPR_U32(ctx, 14, READ32(ADD32(GPR_U32(ctx, 4), 0)));
    // 0x15fa94: 0x24a50040
    ctx->pc = 0x15fa94u;
    SET_GPR_S32(ctx, 5, ADD32(GPR_U32(ctx, 5), 64));
    // 0x15fa98: 0x24c60040
    ctx->pc = 0x15fa98u;
    SET_GPR_S32(ctx, 6, ADD32(GPR_U32(ctx, 6), 64));
    // 0x15fa9c: 0x25ce0080
    ctx->pc = 0x15fa9cu;
    SET_GPR_S32(ctx, 14, ADD32(GPR_U32(ctx, 14), 128));
    // 0x15faa0: 0x316a0001
    ctx->pc = 0x15faa0u;
    SET_GPR_U32(ctx, 10, AND32(GPR_U32(ctx, 11), 1));
    // 0x15faa4: 0x1540ffd4
    ctx->pc = 0x15FAA4u;
    {
        const bool branch_taken_0x15faa4 = (GPR_U32(ctx, 10) != GPR_U32(ctx, 0));
        ctx->pc = 0x15FAA8u;
        SET_GPR_U32(ctx, 11, AND32(GPR_U32(ctx, 11), 65534));
        if (branch_taken_0x15faa4) {
            ctx->pc = 0x15F9F8u;
            goto label_15f9f8;
        }
    }
    ctx->pc = 0x15FAACu;
    // 0x15faac: 0x3e00008
    ctx->pc = 0x15FAACu;
    {
        uint32_t jumpTarget = GPR_U32(ctx, 31);
        ctx->pc = jumpTarget;
        switch (jumpTarget) {
            case 0x15F9F8u: goto label_15f9f8;
            case 0x15FA18u: goto label_15fa18;
            case 0x15FA6Cu: goto label_15fa6c;
            default: break;
        }
        return;
    }
    ctx->pc = 0x15FAB4u;
}
