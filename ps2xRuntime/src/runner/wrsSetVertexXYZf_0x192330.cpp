...
// Address: 0x192330 - 0x192340
void wrsSetVertexXYZf_0x192330(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
#ifdef _DEBUG
    PS_LOG_ENTRY("wrsSetVertexXYZf_0x192330");
#endif

    ctx->pc = 0x192330u;

    // 0x192330: 0xe48c0000  swc1        $f12, 0x0($a0)
    ctx->pc = 0x192330u;
    { float f = ctx->f[12]; uint32_t bits; std::memcpy(&bits, &f, sizeof(bits)); WRITE32(ADD32(GPR_U32(ctx, 4), 0), bits); }
    // 0x192334: 0xe48d0004  swc1        $f13, 0x4($a0)
    ctx->pc = 0x192334u;
    { float f = ctx->f[13]; uint32_t bits; std::memcpy(&bits, &f, sizeof(bits)); WRITE32(ADD32(GPR_U32(ctx, 4), 4), bits); }
    // 0x192338: 0x3e00008  jr          $ra
    ctx->pc = 0x192338u;
    {
        uint32_t jumpTarget = GPR_U32(ctx, 31);
        ctx->pc = 0x19233Cu;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x192338u;
            // 0x19233c: 0xe48e0008  swc1        $f14, 0x8($a0) (Delay Slot)
