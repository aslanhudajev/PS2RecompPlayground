...
void ipbSwapBuffer_0x2aae60(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x2AAE8Cu;
    {
        ...
        {
            static int _swapLog = 0;
            if (++_swapLog <= 20 || (_swapLog % 300) == 0)
                printf("[ipbSwapBuffer #%d] check=0x%x branch=%s\n",
                       _swapLog, GPR_U32(ctx, 2), branch_taken_0x2aae8c ? "PROCEED(kick DMA)" : "SKIP(early return)");
        }
        ctx->pc = 0x2AAE90u;
        SET_GPR_U32(ctx, 2, ((uint32_t)55 << 16));
...
label_2aaea0:
    ...
    {
        static int _swapDbg2 = 0;
        if (++_swapDbg2 <= 20 || (_swapDbg2 % 300) == 0) {
            uint32_t _s0 = GPR_U32(ctx, 16);
            uint32_t _r6 = GPR_U32(ctx, 6);
            printf("[ipbSwapBuffer #%d] s0(disp_struct)=0x%x r6(pbg)=0x%x READ64(s0+464)=0x%llx READ64(s0+480)=0x%llx READ64(s0+488)=0x%llx\n",
                   _swapDbg2, _s0, _r6,
                   (unsigned long long)READ64(ADD32(_s0, 464)),
                   (unsigned long long)READ64(ADD32(_s0, 480)),
                   (unsigned long long)READ64(ADD32(_s0, 488)));
        }
    }
    // 0x2aaed4: 0x3c031200
    ctx->pc = 0x2aaed4u;
