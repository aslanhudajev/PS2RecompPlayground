...
void wrsSetVertexXYZf_0x14ef30(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
#ifdef _DEBUG
    PS_LOG_ENTRY("wrsSetVertexXYZf_0x14ef30");
#endif

    ctx->pc = 0x14ef30u;

    // 0x14ef30: 0xe48c0000  swc1        $f12, 0x0($a0)
    ctx->pc = 0x14ef30u;
    { float f = ctx->f[12]; uint32_t bits; std::memcpy(&bits, &f, sizeof(bits)); WRITE32(ADD32(GPR_U32(ctx, 4), 0), bits); }
    // 0x14ef34: 0xe48d0004  swc1        $f13, 0x4($a0)
    ctx->pc = 0x14ef34u;
    { float f = ctx->f[13]; uint32_t bits; std::memcpy(&bits, &f, sizeof(bits)); WRITE32(ADD32(GPR_U32(ctx, 4), 4), bits); }
    // 0x14ef38: 0x3e00008  jr          $ra
    ctx->pc = 0x14EF38u;
    {
        uint32_t jumpTarget = GPR_U32(ctx, 31);
        ctx->pc = 0x14EF3Cu;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x14EF38u;
            // 0x14ef3c: 0xe48e0008  swc1        $f14, 0x8($a0) (Delay Slot)
