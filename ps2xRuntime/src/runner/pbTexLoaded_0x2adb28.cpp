...
void pbTexLoaded_0x2adb28(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    static int _pbTexLoaded_count = 0;
    ++_pbTexLoaded_count;

    ctx->pc = 0x2adb28u;

    printf("[pbTexLoaded #%d] ENTRY  a0(page)=0x%x  a1(sub)=0x%x  a2(mask)=0x%x  ra=0x%x\n",
        _pbTexLoaded_count, GPR_U32(ctx, 4), GPR_U32(ctx, 5), GPR_U32(ctx, 6), GPR_U32(ctx, 31));
    ...
    ctx->pc = 0x2ADB50u;
    {
        ...
        printf("[pbTexLoaded #%d] bitmap_check: loaded_mask=0x%x => %s  (v0=1 if loaded)\n",
            _pbTexLoaded_count, GPR_U32(ctx, 6), branch_taken_0x2adb50 ? "NOT_LOADED->v0=0" : "LOADED->v0=1");
        ctx->pc = 0x2ADB54u;
...
label_2adb60:
    ...
    ctx->pc = 0x2ADB6Cu;
    // 0x2adb6c: 0x3e00008
    printf("[pbTexLoaded #%d] EXIT  v0=1 (loaded)\n", _pbTexLoaded_count);
    ctx->pc = 0x2ADB6Cu;
    {
...
label_2adb80:
    // 0x2adb80: 0x3e00008
    printf("[pbTexLoaded #%d] EXIT  v0=0 (not loaded)\n", _pbTexLoaded_count);
    ctx->pc = 0x2ADB80u;
    {
        uint32_t jumpTarget = GPR_U32(ctx, 31);
        ctx->pc = 0x2ADB84u;
