...
// GCC libgcc dp-bit.c __unpack_d - unpack IEEE 754 double to fp_number_type
...
{
#ifdef _DEBUG
    PS_LOG_ENTRY("ps2___unpack_d_0x128940");
#endif
    ...
    uint64_t value_raw = READ64(ADD32(srcAddr, 0));
    uint64_t mantissa = value_raw & 0xfffffffffffffull;
    uint64_t exp_bits = (value_raw >> 52) & 0x7ffu;

    WRITE32(ADD32(dstAddr, 4), (value_raw >> 63) & 1); // sign
    ...
    if (exp_bits != 0x7ff)
    {
        WRITE64(ADD32(dstAddr, 12), (mantissa << 8) | 0x1000000000000000ull);
        WRITE32(ADD32(dstAddr, 8), static_cast<int32_t>(exp_bits) - 0x3ff);
    ...
    else if ((value_raw & 0x8000000000000ull) == 0)
    {
