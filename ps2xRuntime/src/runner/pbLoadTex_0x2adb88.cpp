...
void pbLoadTex_0x2adb88(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    static int _pbLT_call = 0;
    ++_pbLT_call;
    auto _rd32 = [&](uint32_t vaddr) -> uint32_t {
        const uint32_t phys = vaddr & PS2_RAM_MASK;
        if (phys + 4u > PS2_RAM_SIZE) return 0xDEAD;
        uint32_t v; std::memcpy(&v, rdram + phys, 4); return v;
    };
    printf("[pbLoadTex #%d ENTRY] a0=0x%x a1=0x%x a2=0x%x ra=0x%x\n",
        _pbLT_call, GPR_U32(ctx, 4), GPR_U32(ctx, 5), GPR_U32(ctx, 6), GPR_U32(ctx, 31));
    printf("  tex_load_page=0x%x mode_flag=0x%x pbg=0x%x\n",
        _rd32(0x3685E0), _rd32(0x3685FC), _rd32(0x35DEE0));
    printf("  page_start[0]=0x%x [1]=0x%x  page_end[0]=0x%x [1]=0x%x\n",
        _rd32(0x368A98), _rd32(0x368A9C), _rd32(0x368AA0), _rd32(0x368AA4));
    printf("  path1_cur=0x%x path1_end=0x%x\n",
        _rd32(0x368A80), _rd32(0x368A88));

    // The game's pbg init leaves page_start/page_end as RDRAM DMA chain pointers
    // (e.g. 0x370820) instead of VRAM block indices (should be < 0x4000).
    // We must fix them every call since other functions keep restoring the bad values.
    {
        auto readLe32 = [&](uint32_t vaddr) -> uint32_t {
            const uint32_t phys = vaddr & PS2_RAM_MASK;
            if (phys + 4u > PS2_RAM_SIZE) return 0;
            uint32_t v;
            std::memcpy(&v, rdram + phys, 4);
            return v;
        };
        auto writeLe32 = [&](uint32_t vaddr, uint32_t value) {
            const uint32_t phys = vaddr & PS2_RAM_MASK;
            if (phys + 4u <= PS2_RAM_SIZE)
                std::memcpy(rdram + phys, &value, 4);
        };
        ...
        constexpr uint32_t VRAM_MAX       = 0x4000u;
        constexpr uint32_t VRAM_BASE      = 0x100u;
        constexpr uint32_t PAGE_SIZE      = ((VRAM_MAX - VRAM_BASE) / 2u) & ~0x1Fu;

        const uint32_t s0 = readLe32(TEX_PAGE_START + 0u);
        const uint32_t s1 = readLe32(TEX_PAGE_START + 4u);
        const uint32_t e0 = readLe32(TEX_PAGE_END   + 0u);
        const uint32_t e1 = readLe32(TEX_PAGE_END   + 4u);
        const uint32_t p1c = readLe32(0x368A80u);
        const uint32_t p1e = readLe32(0x368A88u);

        bool fixNeeded = (s0 >= VRAM_MAX || s1 >= VRAM_MAX ||
                          e0 == 0 || e0 > VRAM_MAX || e1 == 0 || e1 > VRAM_MAX ||
                          p1c >= VRAM_MAX || p1e >= VRAM_MAX);

        if (fixNeeded) {
            if (_pbLT_call <= 30)
                printf("[pbLoadTex #%d] VRAM page reinit: s0=0x%x s1=0x%x e0=0x%x e1=0x%x p1c=0x%x p1e=0x%x\n",
                    _pbLT_call, s0, s1, e0, e1, p1c, p1e);
            writeLe32(TEX_PAGE_START + 0u, VRAM_BASE);
            writeLe32(TEX_PAGE_START + 4u, VRAM_BASE + PAGE_SIZE);
            writeLe32(TEX_PAGE_END   + 0u, VRAM_BASE + PAGE_SIZE);
            writeLe32(TEX_PAGE_END   + 4u, VRAM_BASE + PAGE_SIZE * 2u);
            writeLe32(TEX_LOAD_PAGE, 0u);
            writeLe32(0x368A80u, VRAM_BASE);
            writeLe32(0x368A88u, VRAM_BASE + PAGE_SIZE);
        }
    }
...
label_2adbc8:
    ...
    ctx->pc = 0x2ADBC4u;
    {
        ...
        printf("[pbLoadTex #%d] @2adbc4 busy_check: *(*(pbg+0x2C)+0x74)=0x%x => %s\n",
            _pbLT_call, GPR_U32(ctx, 2), branch_taken_0x2adbc4 ? "BUSY->exit(1)" : "not busy->continue");
        ctx->pc = 0x2ADBC8u;
        SET_GPR_S32(ctx, 2, ADD32(GPR_U32(ctx, 0), 1));
...
label_2adbd8:
    ...
    printf("[pbLoadTex #%d] @2adbd8 s0(page_idx)=%u s3(sub_idx)=%u s1(tex_ptr)=0x%x s2(pbg)=0x%x\n",
        _pbLT_call, GPR_U32(ctx, 16), GPR_U32(ctx, 19), GPR_U32(ctx, 17), GPR_U32(ctx, 18));
label_2adbdc:
    // 0x2adbdc: 0x8e420038
    ctx->pc = 0x2adbdcu;
...
label_2adbf0:
    ...
    ctx->pc = 0x2ADBECu;
    {
        ...
        printf("[pbLoadTex #%d] @2adbec tex_table_check: *(tex_table+s0*16+16)=0x%x => %s\n",
            _pbLT_call, GPR_U32(ctx, 2), branch_taken_0x2adbec ? "ALREADY_IN_TABLE->skip_to_2adc08" : "not_in_table->check_flag");
...
label_2adc00:
    ...
    ctx->pc = 0x2ADBFCu;
    {
        ...
        printf("[pbLoadTex #%d] @2adbfc tex_flag_256: *(s1+8)&256=0x%x => %s\n",
            _pbLT_call, GPR_U32(ctx, 2), branch_taken_0x2adbfc ? "flag_clear->goto_2adc1c" : "flag_set->s0=0,s3=0");
        ctx->pc = 0x2ADC00u;
        SET_GPR_U32(ctx, 2, ((uint32_t)55 << 16));
...
label_2adc1c:
    ...
    printf("[pbLoadTex #%d] @2adc1c tex_load_page(a2/GPR6)=%u  s0(page_idx)=%u  s3(sub_idx)=%u\n",
        _pbLT_call, GPR_U32(ctx, 6), GPR_U32(ctx, 16), GPR_U32(ctx, 19));
label_2adc20:
    // 0x2adc20: 0x8e420038
    ctx->pc = 0x2adc20u;
    SET_GPR_U32(ctx, 2, READ32(ADD32(GPR_U32(ctx, 18), 56)));
...
label_2adc48:
    ...
    ctx->pc = 0x2ADC44u;
    {
        ...
        printf("[pbLoadTex #%d] @2adc44 bitmap_check: bit=%u => %s\n",
            _pbLT_call, GPR_U32(ctx, 2), branch_taken_0x2adc44 ? "ALREADY_LOADED->exit(1)" : "not_loaded->alloc");
        ctx->pc = 0x2ADC48u;
        SET_GPR_S32(ctx, 2, ADD32(GPR_U32(ctx, 0), 1));
...
label_2adc58:
    ...
    ctx->pc = 0x2ADC54u;
    {
        ...
        printf("[pbLoadTex #%d] @2adc54 mode_flag=0x%x => %s\n",
            _pbLT_call, GPR_U32(ctx, 2), branch_taken_0x2adc54 ? "PATH0(page_start/end)" : "PATH1(0x368A80/88)");
        ctx->pc = 0x2ADC58u;
        SET_GPR_U32(ctx, 2, ((uint32_t)55 << 16));
...
label_2adc80:
    ...
    ctx->pc = 0x2ADC7Cu;
    {
        ...
        printf("[pbLoadTex #%d] @2adc7c PATH1_ALLOC: path1_end < alloc_needed=%d => %s  (alloc=0x%x end=0x%x)\n",
            _pbLT_call, GPR_U32(ctx, 2), branch_taken_0x2adc7c ? "NO_ROOM->exit(0)" : "fits->update",
            GPR_U32(ctx, 3), _rd32(0x368A88));
        ctx->pc = 0x2ADC80u;
        SET_GPR_U64(ctx, 2, (uint64_t)GPR_U64(ctx, 0) + (uint64_t)GPR_U64(ctx, 0));
...
label_2adcac:
    ...
    printf("[pbLoadTex #%d] @2adcac PATH0: page_start[%u]=0x%x (VRAM alloc cursor)\n",
        _pbLT_call, GPR_U32(ctx, 6), GPR_U32(ctx, 3));
label_2adcb0:
    // 0x2adcb0: 0xafa3002c
    ctx->pc = 0x2adcb0u;
...
label_2add20:
    // 0x2add20: 0x82102a
    printf("[pbLoadTex #%d] @2add20 PATH0_FIT_CHECK: new_alloc_end(a0)=0x%x vs page_end[%u]=0x%x  tex_size(a1)=0x%x  page_start_base=0x%x\n",
        _pbLT_call, GPR_U32(ctx, 4), GPR_U32(ctx, 6), GPR_U32(ctx, 2), GPR_U32(ctx, 5),
        READ32(ADD32(GPR_U32(ctx, 29), 48)));
    ctx->pc = 0x2add20u;
...
label_2add28:
    ...
    ctx->pc = 0x2ADD24u;
    {
        ...
        printf("[pbLoadTex #%d] @2add24 alloc_fits=%d => %s\n",
            _pbLT_call, branch_taken_0x2add24, branch_taken_0x2add24 ? "ALLOC_OK->write_new_start" : "NO_ROOM->exit(0)");
        ctx->pc = 0x2ADD28u;
        SET_GPR_U32(ctx, 2, ((uint32_t)55 << 16));
...
label_2add30:
    ...
    ctx->pc = 0x2ADD2Cu;
    {
        ...
        printf("[pbLoadTex #%d] @2add2c ALLOC_FAIL! new_alloc >= page_end. v0=0 -> exit FAILURE\n", _pbLT_call);
        printf("  page_start[0]=0x%x [1]=0x%x  page_end[0]=0x%x [1]=0x%x  tex_load_page=%u\n",
            _rd32(0x368A98), _rd32(0x368A9C), _rd32(0x368AA0), _rd32(0x368AA4), _rd32(0x3685E0));
        ctx->pc = 0x2ADD30u;
        SET_GPR_U64(ctx, 2, (uint64_t)GPR_U64(ctx, 0) + (uint64_t)GPR_U64(ctx, 0));
...
label_2addf0:
    ...
    {
        uint32_t _texbyte0 = (uint8_t)READ8(ADD32(GPR_U32(ctx, 17), 0));
        printf("[pbLoadTex #%d] @2addf0 TEX_SETUP: s2(vram_dest)=0x%x  sp[0x2C](page_start_cur)=0x%x  cbp_offset=0x%x  psm_field=0x%x  s0(flags>>4)=0x%x  byte0=0x%x  width=0x%x height=0x%x\n",
            _pbLT_call, GPR_U32(ctx, 18), GPR_U32(ctx, 4), GPR_U32(ctx, 2), GPR_U32(ctx, 3),
            GPR_U32(ctx, 16), _texbyte0,
            (uint16_t)READ16(ADD32(GPR_U32(ctx, 17), 22)),
            (uint16_t)READ16(ADD32(GPR_U32(ctx, 17), 24)));
    }
label_2addf4:
    // 0x2addf4: 0x32020008
    ctx->pc = 0x2addf4u;
...
label_2addfc:
    ...
    ctx->pc = 0x2ADDF8u;
    {
        ...
        printf("[pbLoadTex #%d] @2addf8 PSM_CLUT_CHECK: s0&8=%u => %s\n",
            _pbLT_call, GPR_U32(ctx, 2), branch_taken_0x2addf8 ? "NO_CLUT->goto_2ade94" : "HAS_CLUT->setup_clut");
        ctx->pc = 0x2ADDFCu;
        SET_GPR_U32(ctx, 2, ((uint32_t)59 << 16));
...
label_2ade98:
    ...
    ctx->pc = 0x2ADE94u;
    printf("[pbLoadTex #%d] @2ade94 FORMAT_SWITCH: s0=%u s7(clut_base)=0x%x s2(vram)=0x%x s1(texptr)=0x%x\n",
        _pbLT_call, GPR_U32(ctx, 16), GPR_U32(ctx, 23), GPR_U32(ctx, 18), GPR_U32(ctx, 17));
    {
...
label_2adf4c:
    ...
    ctx->pc = 0x2ADF4Cu;
    SET_GPR_U32(ctx, 10, READ32(ADD32(GPR_U32(ctx, 2), 0)));
    printf("[pbLoadTex #%d] @2adf48 CALL TexDmaAppendTexture(1): a0=0x%x a1=0x%x a2=0x%x a3=0x%x t0=0x%x t1=0x%x t2=0x%x s2_vram=0x%x\n",
        _pbLT_call, GPR_U32(ctx, 4), GPR_U32(ctx, 5), GPR_U32(ctx, 6), GPR_U32(ctx, 7),
        GPR_U32(ctx, 8), GPR_U32(ctx, 9), GPR_U32(ctx, 10), GPR_U32(ctx, 18));
    ctx->pc = 0x2AD5C8u;
    {
        const uint32_t __entryPc = ctx->pc;
...
label_2adfcc:
    ...
    ctx->pc = 0x2ADFCCu;
    SET_GPR_U32(ctx, 10, READ32(ADD32(GPR_U32(ctx, 29), 36)));
    printf("[pbLoadTex #%d] @2adfc8 CALL TexDmaAppendTexture(2): a0=0x%x a1=0x%x a2=0x%x a3=0x%x t0=0x%x t1=0x%x t2=0x%x s2_vram=0x%x\n",
        _pbLT_call, GPR_U32(ctx, 4), GPR_U32(ctx, 5), GPR_U32(ctx, 6), GPR_U32(ctx, 7),
        GPR_U32(ctx, 8), GPR_U32(ctx, 9), GPR_U32(ctx, 10), GPR_U32(ctx, 18));
    ctx->pc = 0x2AD5C8u;
    {
...
label_2ae0cc:
    ...
    ctx->pc = 0x2AE0CCu;
    SET_GPR_U64(ctx, 10, (uint64_t)GPR_U64(ctx, 21) + (uint64_t)GPR_U64(ctx, 0));
    printf("[pbLoadTex #%d] @2ae0c8 CALL TexDmaAppendTexture(3): a0=0x%x a1=0x%x a2=0x%x a3=0x%x t0=0x%x t1=0x%x t2=0x%x s2_vram=0x%x s3_iter=%u\n",
        _pbLT_call, GPR_U32(ctx, 4), GPR_U32(ctx, 5), GPR_U32(ctx, 6), GPR_U32(ctx, 7),
        GPR_U32(ctx, 8), GPR_U32(ctx, 9), GPR_U32(ctx, 10), GPR_U32(ctx, 18), GPR_U32(ctx, 19));
    ctx->pc = 0x2AD5C8u;
    {
        const uint32_t __entryPc = ctx->pc;
        TexDmaAppendTexture_0x2ad5c8(rdram, ctx, runtime);
...
label_2ae13c:
    ...
    ctx->pc = 0x2AE13Cu;
    SET_GPR_U32(ctx, 9, READ32(ADD32(GPR_U32(ctx, 29), 48)));
    printf("[pbLoadTex #%d] @2ae138 CALL pbTexTexXfer: a0=0x%x a1=0x%x a2=0x%x a3=0x%x t0=0x%x t1=0x%x s2_vram=0x%x\n",
        _pbLT_call, GPR_U32(ctx, 4), GPR_U32(ctx, 5), GPR_U32(ctx, 6), GPR_U32(ctx, 7),
        GPR_U32(ctx, 8), GPR_U32(ctx, 9), GPR_U32(ctx, 18));
    ctx->pc = 0x2AD8F0u;
    {
        const uint32_t __entryPc = ctx->pc;
...
label_2ae184:
    // 0x2ae184: 0x24020001
    printf("[pbLoadTex #%d] @2ae184 ALLOC_SUCCESS -> v0=1\n", _pbLT_call);
    ctx->pc = 0x2ae184u;
    SET_GPR_S32(ctx, 2, ADD32(GPR_U32(ctx, 0), 1));
label_2ae188:
    printf("[pbLoadTex #%d EXIT] v0=%u (%s)  page_start[0]=0x%x [1]=0x%x  page_end[0]=0x%x [1]=0x%x  tex_load_page=%u  mode_flag=0x%x\n",
        _pbLT_call, GPR_U32(ctx, 2), GPR_U32(ctx, 2) ? "SUCCESS" : "FAIL",
        _rd32(0x368A98), _rd32(0x368A9C), _rd32(0x368AA0), _rd32(0x368AA4), _rd32(0x3685E0), _rd32(0x3685FC));
    // 0x2ae188: 0xdfbf00d0
    ctx->pc = 0x2ae188u;
