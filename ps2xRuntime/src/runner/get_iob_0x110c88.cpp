#include "ps2_runtime_macros.h"
#include "ps2_runtime.h"
#include "ps2_recompiled_functions.h"
#include "ps2_recompiled_stubs.h"

#include "ps2_syscalls.h"
#include "ps2_stubs.h"

#ifdef _DEBUG
#include "ps2_log.h"
#endif

// Function: get_iob
// Address: 0x110c88 - 0x110cf4
void get_iob_0x110c88(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
#ifdef _DEBUG
    PS_LOG_ENTRY("get_iob_0x110c88");
#endif

    ctx->pc = 0x110c88u;

    // 0x110c88: 0x27bdffd0  addiu       $sp, $sp, -0x30
    ctx->pc = 0x110c88u;
    SET_GPR_S32(ctx, 29, (int32_t)ADD32(GPR_U32(ctx, 29), 4294967248));
    // 0x110c8c: 0xffb00000  sd          $s0, 0x0($sp)
    ctx->pc = 0x110c8cu;
    WRITE64(ADD32(GPR_U32(ctx, 29), 0), GPR_U64(ctx, 16));
    // 0x110c90: 0xffb10010  sd          $s1, 0x10($sp)
    ctx->pc = 0x110c90u;
    WRITE64(ADD32(GPR_U32(ctx, 29), 16), GPR_U64(ctx, 17));
    // 0x110c94: 0x80802d  daddu       $s0, $a0, $zero
    ctx->pc = 0x110c94u;
    SET_GPR_U64(ctx, 16, (uint64_t)GPR_U64(ctx, 4) + (uint64_t)GPR_U64(ctx, 0));
    // 0x110c98: 0xffbf0020  sd          $ra, 0x20($sp)
    ctx->pc = 0x110c98u;
    WRITE64(ADD32(GPR_U32(ctx, 29), 32), GPR_U64(ctx, 31));
    // 0x110c9c: 0xc0442e8  jal         func_110BA0
    ctx->pc = 0x110C9Cu;
    SET_GPR_U32(ctx, 31, 0x110CA4u);
    ctx->pc = 0x110CA0u;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x110C9Cu;
            // 0x110ca0: 0x3c110017  lui         $s1, 0x17 (Delay Slot)
        SET_GPR_S32(ctx, 17, (int32_t)((uint32_t)23 << 16));
        ctx->in_delay_slot = false;
    ctx->pc = 0x110BA0u;
    if (runtime->hasFunction(0x110BA0u)) {
        auto targetFn = runtime->lookupFunction(0x110BA0u);
        const uint32_t __entryPc = ctx->pc;
        targetFn(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x110CA4u; }
        if (ctx->pc != 0x110CA4u) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
        _sceFsIobSemaMK_0x110ba0(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x110CA4u; }
        if (ctx->pc != 0x110CA4u) { return; }
    }
    ctx->pc = 0x110CA4u;
    // 0x110ca4: 0xc0438b8  jal         func_10E2E0
    ctx->pc = 0x110CA4u;
    SET_GPR_U32(ctx, 31, 0x110CACu);
    ctx->pc = 0x110CA8u;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x110CA4u;
            // 0x110ca8: 0x8e24fbc0  lw          $a0, -0x440($s1) (Delay Slot)
        SET_GPR_S32(ctx, 4, (int32_t)READ32(ADD32(GPR_U32(ctx, 17), 4294966208)));
        ctx->in_delay_slot = false;
    ctx->pc = 0x10E2E0u;
    if (runtime->hasFunction(0x10E2E0u)) {
        auto targetFn = runtime->lookupFunction(0x10E2E0u);
        const uint32_t __entryPc = ctx->pc;
        targetFn(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x110CACu; }
        if (ctx->pc != 0x110CACu) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
        WaitSema_0x10e2e0(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x110CACu; }
        if (ctx->pc != 0x110CACu) { return; }
    }
    ctx->pc = 0x110CACu;
    // 0x110cac: 0x2e030020  sltiu       $v1, $s0, 0x20
    ctx->pc = 0x110cacu;
    SET_GPR_U64(ctx, 3, ((uint64_t)GPR_U64(ctx, 16) < (uint64_t)(int64_t)(int32_t)32) ? 1 : 0);
    // 0x110cb0: 0x14600005  bnez        $v1, . + 4 + (0x5 << 2)
    ctx->pc = 0x110CB0u;
    {
        const bool branch_taken_0x110cb0 = (GPR_U64(ctx, 3) != GPR_U64(ctx, 0));
        ctx->pc = 0x110CB4u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x110CB0u;
            // 0x110cb4: 0x3c020018  lui         $v0, 0x18 (Delay Slot)
        SET_GPR_S32(ctx, 2, (int32_t)((uint32_t)24 << 16));
        ctx->in_delay_slot = false;
        if (branch_taken_0x110cb0) {
            ctx->pc = 0x110CC8u;
            goto label_110cc8;
        }
    }
    ctx->pc = 0x110CB8u;
    // 0x110cb8: 0xc0438b0  jal         func_10E2C0
    ctx->pc = 0x110CB8u;
    SET_GPR_U32(ctx, 31, 0x110CC0u);
    ctx->pc = 0x110CBCu;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x110CB8u;
            // 0x110cbc: 0x8e24fbc0  lw          $a0, -0x440($s1) (Delay Slot)
        SET_GPR_S32(ctx, 4, (int32_t)READ32(ADD32(GPR_U32(ctx, 17), 4294966208)));
        ctx->in_delay_slot = false;
    ctx->pc = 0x10E2C0u;
    if (runtime->hasFunction(0x10E2C0u)) {
        auto targetFn = runtime->lookupFunction(0x10E2C0u);
        const uint32_t __entryPc = ctx->pc;
        targetFn(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x110CC0u; }
        if (ctx->pc != 0x110CC0u) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
        SignalSema_0x10e2c0(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x110CC0u; }
        if (ctx->pc != 0x110CC0u) { return; }
    }
    ctx->pc = 0x110CC0u;
    // 0x110cc0: 0x10000007  b           . + 4 + (0x7 << 2)
    ctx->pc = 0x110CC0u;
    {
        const bool branch_taken_0x110cc0 = (GPR_U64(ctx, 0) == GPR_U64(ctx, 0));
        ctx->pc = 0x110CC4u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x110CC0u;
            // 0x110cc4: 0x102d  daddu       $v0, $zero, $zero (Delay Slot)
        SET_GPR_U64(ctx, 2, (uint64_t)GPR_U64(ctx, 0) + (uint64_t)GPR_U64(ctx, 0));
        ctx->in_delay_slot = false;
        if (branch_taken_0x110cc0) {
            ctx->pc = 0x110CE0u;
            goto label_110ce0;
        }
    }
    ctx->pc = 0x110CC8u;
label_110cc8:
    // 0x110cc8: 0x108100  sll         $s0, $s0, 4
    ctx->pc = 0x110cc8u;
    SET_GPR_S32(ctx, 16, (int32_t)SLL32(GPR_U32(ctx, 16), 4));
    // 0x110ccc: 0x244297c0  addiu       $v0, $v0, -0x6840
    ctx->pc = 0x110cccu;
    SET_GPR_S32(ctx, 2, (int32_t)ADD32(GPR_U32(ctx, 2), 4294940608));
    // 0x110cd0: 0x8e24fbc0  lw          $a0, -0x440($s1)
    ctx->pc = 0x110cd0u;
    SET_GPR_S32(ctx, 4, (int32_t)READ32(ADD32(GPR_U32(ctx, 17), 4294966208)));
    // 0x110cd4: 0xc0438b0  jal         func_10E2C0
    ctx->pc = 0x110CD4u;
    SET_GPR_U32(ctx, 31, 0x110CDCu);
    ctx->pc = 0x110CD8u;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x110CD4u;
            // 0x110cd8: 0x2028021  addu        $s0, $s0, $v0 (Delay Slot)
        SET_GPR_S32(ctx, 16, (int32_t)ADD32(GPR_U32(ctx, 16), GPR_U32(ctx, 2)));
        ctx->in_delay_slot = false;
    ctx->pc = 0x10E2C0u;
    if (runtime->hasFunction(0x10E2C0u)) {
        auto targetFn = runtime->lookupFunction(0x10E2C0u);
        const uint32_t __entryPc = ctx->pc;
        targetFn(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x110CDCu; }
        if (ctx->pc != 0x110CDCu) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
        SignalSema_0x10e2c0(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x110CDCu; }
        if (ctx->pc != 0x110CDCu) { return; }
    }
    ctx->pc = 0x110CDCu;
    // 0x110cdc: 0x200102d  daddu       $v0, $s0, $zero
    ctx->pc = 0x110cdcu;
    SET_GPR_U64(ctx, 2, (uint64_t)GPR_U64(ctx, 16) + (uint64_t)GPR_U64(ctx, 0));
label_110ce0:
    // 0x110ce0: 0xdfbf0020  ld          $ra, 0x20($sp)
    ctx->pc = 0x110ce0u;
    SET_GPR_U64(ctx, 31, READ64(ADD32(GPR_U32(ctx, 29), 32)));
    // 0x110ce4: 0xdfb10010  ld          $s1, 0x10($sp)
    ctx->pc = 0x110ce4u;
    SET_GPR_U64(ctx, 17, READ64(ADD32(GPR_U32(ctx, 29), 16)));
    // 0x110ce8: 0xdfb00000  ld          $s0, 0x0($sp)
    ctx->pc = 0x110ce8u;
    SET_GPR_U64(ctx, 16, READ64(ADD32(GPR_U32(ctx, 29), 0)));
    // 0x110cec: 0x3e00008  jr          $ra
    ctx->pc = 0x110CECu;
    {
        uint32_t jumpTarget = GPR_U32(ctx, 31);
        ctx->pc = 0x110CF0u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x110CECu;
            // 0x110cf0: 0x27bd0030  addiu       $sp, $sp, 0x30 (Delay Slot)
        SET_GPR_S32(ctx, 29, (int32_t)ADD32(GPR_U32(ctx, 29), 48));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        switch (jumpTarget) {
            case 0x110CC8u: goto label_110cc8;
            case 0x110CE0u: goto label_110ce0;
            default: break;
        }
        return;
    }
    ctx->pc = 0x110CF4u;
}
