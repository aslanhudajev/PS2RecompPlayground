...
#include "ps2_stubs.h"
#include <cstdio>
#define _MEM(addr) READ32((addr))
#define _LOG(tag) printf("[ps2_main] " tag " pc=%x ra=%x sp=%x c0=%u 98=%u 84=%u v0=%x s2=%x s3=%x\n", ctx->pc, GPR_U32(ctx,31), GPR_U32(ctx,29), _MEM(0x30F1C0u), _MEM(0x30F198u), _MEM(0x30F184u), GPR_U32(ctx,2), GPR_U32(ctx,18), GPR_U32(ctx,19))
#define _LOG_ARGS() printf("[ps2_main]   args a0=%x a1=%x a2=%x a3=%x\n", GPR_U32(ctx,4), GPR_U32(ctx,5), GPR_U32(ctx,6), GPR_U32(ctx,7))
#define _LOG_RET() printf("[ps2_main]   ret v0=%x s2=%x c0=%u 98=%u\n", GPR_U32(ctx,2), GPR_U32(ctx,18), _MEM(0x30F1C0u), _MEM(0x30F198u))
...
label_200388:
    ...
    ctx->pc = 0x2003FCu;
    // 0x2003fc: 0xc0b179a  jal MBEndFrame (first call)
    ctx->pc = 0x2003FCu;
    ...
    printf("[ps2_main] <<<MB1 pc=%x v0=%x s2=%x c0=%u 98=%u\n", ctx->pc, GPR_U32(ctx,2), GPR_U32(ctx,18), _MEM(0x30F1C0u), _MEM(0x30F198u));
    if (ctx->pc != 0x200404u) { return; }
    ctx->pc = 0x200404u;
    // 0x200404: 0xc0b179a  jal MBEndFrame (second call)
    ctx->pc = 0x200404u;
    ...
    printf("[ps2_main] <<<MB2 pc=%x v0=%x s2=%x c0=%u 98=%u 84=%u\n", ctx->pc, GPR_U32(ctx,2), GPR_U32(ctx,18), _MEM(0x30F1C0u), _MEM(0x30F198u), _MEM(0x30F184u));
...
label_200658:
    ...
    // FORCE: set view_mode=1 so MainMenu path is taken
    // WRITE32(0x30F198u, 1u);
    // 0x2006a8: 0x8e22f198
    ctx->pc = 0x2006a8u;
