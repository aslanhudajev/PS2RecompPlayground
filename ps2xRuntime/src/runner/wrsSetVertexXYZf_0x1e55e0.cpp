...
// Address: 0x1e55e0 - 0x1e55f0
void wrsSetVertexXYZf_0x1e55e0(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
#ifdef _DEBUG
    PS_LOG_ENTRY("wrsSetVertexXYZf_0x1e55e0");
#endif

    ctx->pc = 0x1e55e0u;

    // 0x1e55e0: 0xe48c0000  swc1        $f12, 0x0($a0)
    ctx->pc = 0x1e55e0u;
    { float f = ctx->f[12]; uint32_t bits; std::memcpy(&bits, &f, sizeof(bits)); WRITE32(ADD32(GPR_U32(ctx, 4), 0), bits); }
    // 0x1e55e4: 0xe48d0004  swc1        $f13, 0x4($a0)
    ctx->pc = 0x1e55e4u;
    { float f = ctx->f[13]; uint32_t bits; std::memcpy(&bits, &f, sizeof(bits)); WRITE32(ADD32(GPR_U32(ctx, 4), 4), bits); }
    // 0x1e55e8: 0x3e00008  jr          $ra
    ctx->pc = 0x1E55E8u;
    {
        uint32_t jumpTarget = GPR_U32(ctx, 31);
        ctx->pc = 0x1E55ECu;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x1E55E8u;
            // 0x1e55ec: 0xe48e0008  swc1        $f14, 0x8($a0) (Delay Slot)
