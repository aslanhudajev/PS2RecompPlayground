...
void pbFatalErrorf_0x2a5628(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    static int _pbFatal_count = 0;
    ++_pbFatal_count;

    ctx->pc = 0x2a5628u;

    printf("[pbFatalErrorf #%d] ENTRY  a0(fmt)=0x%x  a1-a3=0x%x 0x%x 0x%x  ra=0x%x  sp=0x%x\n",
        _pbFatal_count, GPR_U32(ctx, 4), GPR_U32(ctx, 5), GPR_U32(ctx, 6), GPR_U32(ctx, 7),
