...
void TexDmaAppendTexture_0x2ad5c8(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    static int _texDma_count = 0;
    ++_texDma_count;

    ctx->pc = 0x2ad5c8u;

    printf("[TexDmaAppendTexture #%d] ENTRY  a0=0x%x a1=0x%x a2=0x%x a3=0x%x  t0=0x%x t1=0x%x t2=0x%x\n",
        _texDma_count, GPR_U32(ctx, 4), GPR_U32(ctx, 5), GPR_U32(ctx, 6), GPR_U32(ctx, 7),
        GPR_U32(ctx, 8), GPR_U32(ctx, 9), GPR_U32(ctx, 10));
