...
#ifdef _DEBUG
    PS_LOG_ENTRY("DRAW_setDisplayBuffer_0x108ec0");
#endif

    ctx->pc = 0x108ec0u;

    // 0x108ec0: 0x30840001  andi        $a0, $a0, 0x1
    ctx->pc = 0x108ec0u;
    SET_GPR_VEC(ctx, 4, PS2_PAND(GPR_VEC(ctx, 4), _mm_cvtsi32_si128((int)1u)));
    // 0x108ec4: 0x27bdfff0  addiu       $sp, $sp, -0x10
    ctx->pc = 0x108ec4u;
    SET_GPR_S32(ctx, 29, (int32_t)ADD32(GPR_U32(ctx, 29), 4294967280));
    // 0x108ec8: 0xa7848494  sh          $a0, -0x7B6C($gp)
    ctx->pc = 0x108ec8u;
    WRITE16(ADD32(GPR_U32(ctx, 28), 4294935700), (uint16_t)GPR_U32(ctx, 4));
    // 0x108ecc: 0xffbf0000  sd          $ra, 0x0($sp)
    ctx->pc = 0x108eccu;
    WRITE64(ADD32(GPR_U32(ctx, 29), 0), GPR_U64(ctx, 31));
    // 0x108ed0: 0xc0432fc  jal         func_10CBF0
    ctx->pc = 0x108ED0u;
    SET_GPR_U32(ctx, 31, 0x108ED8u);
    ctx->pc = 0x108ED4u;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x108ED0u;
            // 0x108ed4: 0x202d  daddu       $a0, $zero, $zero (Delay Slot)
